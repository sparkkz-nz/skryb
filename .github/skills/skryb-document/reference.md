...
| `layout` | No | Flowchart only. Places nodes that have no `position`. |
...
loaded and some node has no position, assigns positions to exactly those nodes,
and the result is baked into the model immediately. Saving writes the actual
current positions, so there is nothing left to do on a second open, layout can
never fight a hand edit, and there is no `layout: manual` and no state to track.
In practice it matters exactly once: the first time an agent-authored file is
opened.

**Keep the `layout` key after baking.** It becomes a no-op for a fully
positioned diagram, but it declares *how to place anything unpositioned*, so a
node appended later with no coordinates is placed on the next open. That is what
makes incremental edits work without re-laying out the whole diagram.

The rules:

- **`layout` absent is exactly today's behaviour.** Every existing document is
  unaffected, and a node with no position keeps the current fallback rather than
  being laid out.
...
re-flowed. Every edge carries both anchors, so it already states a spatial
relationship, and a node sits on the side its own anchor faces away from: a
connector entering the new node's `left` means the neighbour is to its left, so
the node goes to that neighbour's right, centred on it and one stage gap clear.
Several connectors are reconciled by clearing every neighbour along the flow and
sitting between them across it.
